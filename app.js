...
class QuizUI {
    ...
    renderMajors(track) {
        // نختار تخصصين من العلمي وتخصصين من الأدبي دون إظهار ذلك للمستخدم.
        const scientific = this.engine.shuffle([...(track.majors.scientific || [])]);
        const literary = this.engine.shuffle([...(track.majors.literary || [])]);

        const fromScientific = scientific.slice(0, 2);
        const fromLiterary = literary.slice(0, 2);

        // إذا نقص أحد الجانبين نكمل من الآخر حتى نصل إلى 4 إن أمكن.
        let suggested = [...fromScientific, ...fromLiterary];
        if (suggested.length < 4) {
            const usedNames = new Set(suggested.map(m => m.name));
            const extras = [...scientific, ...literary].filter(m => !usedNames.has(m.name));
            suggested = [...suggested, ...extras].slice(0, 4);
        }

        // خلط الترتيب حتى لا يظهر التقسيم بين العلمي والأدبي.
