   ...
   setUserData(name, phone, age, academicBranch) {
    ...
    computeResult() {
        ...
        const percentages = Object.fromEntries(keys.map(k => [
            k, opportunities[k] ? Math.round(10 + ((scores[k] / opportunities[k]) * 90)) : 10
        ]));
        ...
        return { best: ranking[0], percentages, ranking, confidence, skipReasons: this.skipReasons || [] };
    }
}
...
class QuizUI {
    ...
    bindEvents() {
            ...
            this.submitChoiceBtn.addEventListener('click', () => {
                ...
                if (!shouldSkipFormSubmit(phone)) {
                    postGoogleForm(
                        ...
                        { 'entry.447393548': phone, 'entry.849275435': branch }
                    );
        ...
        if(this.startQuizFormBtn) {
            this.startQuizFormBtn.addEventListener('click', () => {
                ...
                if (!shouldSkipFormSubmit(phone)) {
                    const fieldsOld = {
                        'entry.1107585805': name,
                        'entry.124913972': phone,
                        'entry.1983079251': age,
                        'entry.1746227930': unionChoice
                    };
                    if (academicBranch) fieldsOld['entry.234839250'] = academicBranch;
                    postGoogleForm(
                        'https://docs.google.com/forms/d/e/1FAIpQLSdNUYMpALSi4m2sdl99a1IiTPPerAvXGutEcJNnaWrAsYWHKw/formResponse',
                        fieldsOld
                    );
                    const fieldsNew = {
                        'entry.847679888': name,
                        'entry.985614909': phone,
                        'entry.55727980': age,
                        'entry.1660430982': unionChoice
                    };
                    if (academicBranch) fieldsNew['entry.1598693234'] = academicBranch;
                    postGoogleForm(
                        'https://docs.google.com/forms/d/e/1FAIpQLSd8Kau8gogYPE0kgVIwQ5SLJjH553BDacjZwFrGyb8KD5rbdA/formResponse',
                        fieldsNew
                    );
                    markFormSubmitted(phone);
                }
    ...
    openSkipReasonPicker() {
        ...
        overlay.innerHTML = `
            <div class="skip-reason-card" role="dialog" aria-modal="true" aria-label="سبب التخطي">
                ...
                <div class="skip-reason-actions">
                    ...
                    <button type="button" class="btn" data-reason="أسباب أخرى">أسباب أخرى</button>
                </div>
        ...
        overlay.querySelectorAll('[data-reason]').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (this.engine.currentIndex < QUESTION_COUNT - 1) {
                    this.engine.next(); this.updateUI();
    ...
    async showResult(restoring = false) {
        ...
        const result = restoring && this.engine.result ? this.engine.result : this.engine.computeResult();
        ...
        let firstName = "";
        if (this.engine.userData?.name) firstName = this.engine.userData.name.trim().split(' ')[0];
        if (this.resultPill && firstName) this.resultPill.textContent = `🎉 نتيجتك جاهزة يا ${firstName}`;
        ...
        return new Promise((resolve) => {
            ...
            const showStep = () => {
                if (i < steps.length) {
                    ...
                    i++; setTimeout(showStep, 900);
                } else setTimeout(() => { overlay.classList.remove('active'); document.body.style.overflow = ''; resolve(); }, 400);
            };
            showStep();
        });
    }
    ...
    renderSkipSummary(result) {
        ...
        if (!box) {
            ...
            if (this.confidenceLabel?.parentNode) this.confidenceLabel.parentNode.insertBefore(box, this.confidenceLabel.nextSibling);
            else this.resultBox?.querySelector('.resultBox')?.appendChild(box);
        }
        const skips = (result && result.skipReasons) || this.engine.skipReasons || [];
        if (!skips.length) { box.classList.add('hidden'); box.innerHTML = ''; return; }
        const counts = {};
        skips.forEach(s => { counts[s.reason || 'بدون سبب'] = (counts[s.reason || 'بدون سبب'] || 0) + 1; });
        const lines = Object.entries(counts).map(([reason, n]) => `<li><b>${reason}</b> — ${n} ${n === 1 ? 'سؤال' : 'أسئلة'}</li>`).join('');
        box.classList.remove('hidden');
        box.innerHTML = `<div class="skip-summary-title">ملاحظات التخطي</div><p class="skip-summary-help">تخطيت ${skips.length} من الأسئلة. هذا قد يجعل النتيجة أقل دقة قليلاً.</p><ul class="skip-summary-list">${lines}</ul>`;
    }
    ...
    renderMajors(track) {
        ...
        const isTooSimilar = (a, b) => {
            ...
            return shared >= 2 && shared / Math.min(ta.length, tb.length) >= 0.6;
        };
        const conflictsWithPicked = (major, picked) => picked.some(p => isTooSimilar(p.name, major.name));
        ...
        const relevance = (major) => {
            ...
            return tokensOf(major.name).reduce((s, t) => s + (branchHint.includes(t) ? 1 : 0), 0);
        };
        const rankPool = (list) => this.engine.shuffle([...(list || [])]).sort((a, b) => relevance(b) - relevance(a));
        ...
        if (suggested.length < 4) {
            const pool = [...(track.majors.scientific || []), ...(track.majors.literary || [])];
        ...
        this.majorsList.querySelectorAll('.major').forEach((btn, index) => {
            btn.addEventListener('click', () => this.openMajorModal(suggested[index]));
        });
    }
...
class NotesFeedback {
    constructor(ui) {
        ...
        this.formUrl = 'https://docs.google.com/forms/d/e/1FAIpQLScKu23LHukvNt_aT_XwvRKkMR9Y-JK5zeGbkFmwzAH865d1cA/formResponse';
    ...
    bindEvents() {
        ...
        if (this.modal) this.modal.addEventListener('click', (e) => { if (e.target === this.modal) this.closeNoteModal(); });
    }
    openNoteModal() {
    ...
    submitNote() {
        ...
        if (this.submitBtn) { this.submitBtn.disabled = true; this.submitBtn.textContent = 'جارٍ الإرسال...'; }
        const payload = new URLSearchParams();
        payload.append(this.entryId, textVal);
        // نفس أسلوب نماذج التسجيل الناجحة + إرسال مباشر
        fetch(this.formUrl, {
            method: 'POST',
            mode: 'no-cors',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' },
            body: payload.toString()
        }).catch(e => console.error('notes form', e))
        .finally(() => {
            this.submitting = false;
            if (this.submitBtn) { this.submitBtn.disabled = false; this.submitBtn.textContent = original || 'إرسال'; }
