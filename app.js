...
class NotesFeedback {
    ...
    bindEvents() {
        if (this.notesBtn) {
            this.notesBtn.addEventListener('click', () => this.openNoteModal());
        }
        if (this.closeBtn) {
    ...
    submitNote() {
        ...
        const payload = new URLSearchParams();
        payload.append(this.entryId, text);
        ...
        fetch(this.formUrl, {
            ...
            // no-cors: لا نعرف نجاح الاستجابة بدقة؛ نعتبر الإرسال تم من جهة العميل.
        }).finally(() => {
            this.submitting = false;
            if (this.submitBtn) {
