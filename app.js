...
// حماية من تكرار إرسال Google Form لنفس الرقم خلال فترة قصيرة (5 دقائق).
...
class QuizUI {
    ...
    bindEvents() {
        ...
        if(this.startQuizFormBtn) {
            this.startQuizFormBtn.addEventListener('click', () => {
                ...
                // 2. إرسال البيانات إلى Google Form مع حماية من التكرار لنفس الرقم خلال 5 دقائق.
                if (!shouldSkipFormSubmit(phone)) {
                    const payload = new URLSearchParams();
    ...
    renderMajors(track) {
        // توحيد الاسم لتجنّب التكرار والتشابه رغم اختلاف الإيموجي أو المسافات.
        ...
        // منع نفس الاختصاص أو أسماء متقاربة جداً (مثل هندسة البرمجيات / هندسة البرمجيات ونظم المعلومات).
        ...
        // تفضيل صامت للتخصصات القريبة مما كتبه الطالب في الفرع الأكاديمي (بدون أي رسالة له).
        ...
        const relevance = (major) => {
            ...
            const tokens = tokensOf(major.name);
            let score = 0;
        ...
        const rankPool = (list) => {
            const arr = [...(list || [])];
            // خلط أولاً ثم ترتيب حسب الصلة حتى لا يظهر ترتيب ثابت.
        ...
        // تخصصان من العلمي + تخصصان من الأدبي، بدون تكرار/تشابه وبدون إظهار الفرع.
        ...
        // إن نقص العدد نكمّل من الباقي دون تكرار أو تشابه.
...
class ResultExporter {
    ...
    async buildImage() {
        ...
        // لا نعرض «كلية/معهد» ولا نص «اضغط للتفاصيل» في صورة المشاركة.
        ...
        majors.slice(0, 4).forEach((major, i) => {
            ...
            // توسيط الاسم عمودياً عندما لا يوجد سطر meta.
