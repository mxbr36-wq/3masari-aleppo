...
class QuizUI {
    ...
    renderMajors(track) {
        // توحيد الاسم لتجنّب تكرار نفس الاختصاص رغم اختلاف الإيموجي أو المسافات.
        ...
        const pickUnique = (list, count, used) => {
            const out = [];
            for (const m of this.engine.shuffle([...(list || [])])) {
        ...
        const used = new Set();
        // تخصصان من العلمي + تخصصان من الأدبي، بدون تكرار وبدون إظهار الفرع للمستخدم.
        const fromScientific = pickUnique(track.majors.scientific, 2, used);
        const fromLiterary = pickUnique(track.majors.literary, 2, used);

        let suggested = [...fromScientific, ...fromLiterary];
        // إن نقص العدد نكمّل من الباقي دون تكرار.
        if (suggested.length < 4) {
            const pool = [
                ...(track.majors.scientific || []),
                ...(track.majors.literary || [])
            ];
            suggested = [...suggested, ...pickUnique(pool, 4 - suggested.length, used)];
        }
...
class ResultExporter {
    ...
    async buildImage() {
        ...
        majors.slice(0, 4).forEach((major, i) => {
            ...
            // توسيط الاسم عمودياً لأننا لم نعد نعرض سطر الـ meta.
            ctx.fillStyle = '#183f3c'; ctx.font = '700 24px system-ui'; ctx.fillText(major, 930, y + 38);
            if (metas[i]) {
